...
def tampilkan_jadwal(jadwal):
    ...
    for i, j in enumerate(jadwal):
        print(f"{i+1}. {j['Maskapai']} | {j['Asal']} → {j['Tujuan']} | {j['Tanggal']} | Rp{j['Harga']}")


def beli_tiket(jadwal):
    tampilkan_jadwal(jadwal)
    try:
        pilih = int(input("Pilih nomor penerbangan: ")) - 1
        ...
        with open(RIWAYAT_FILE, mode='a', newline='') as file:
            ...
            writer.writerow([nama, data['Maskapai'], data['Asal'], data['Tujuan'], data['Tanggal'], jumlah, total])
...
def main():
    ...
    while True:
        ...
        print("2. Beli Tiket")
        print("3. Lihat Tiket yang Dibeli")
